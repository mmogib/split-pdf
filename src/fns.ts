...
export const getFilePaths = (
  ...
  const entryPaths = fs
    ...
    .filter((entry: string) => !ignore.includes(entry))
    .map((entry: string) => path.join(folderPath, entry));

  const filesOnly = entryPaths.filter((entryPath: string) =>
    fs.statSync(entryPath).isFile()
  );
  const directoriesOnly = entryPaths.filter(
    (entryPath: string) => !filesOnly.includes(entryPath)
  );

  return include_files ? [...filesOnly, ...directoriesOnly] : directoriesOnly;
};

export const getPathsForSpecificExtensions = (
  folderPath: string,
  Extensions: string[]
): filePath[] => {
  const entryPaths = fs
